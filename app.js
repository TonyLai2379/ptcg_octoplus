...
} catch (e) { console.error("Supabase 初始化發生例外錯誤", e); }
...
function applyAutoZoom() {
    let width = window.innerWidth; let targetZoom = 1;
    if (width < 1400) targetZoom = 0.9;
    if (width < 1200) targetZoom = 0.8;
    if (width < 992) targetZoom = 0.7;
    if (width < 768) targetZoom = 0.55;
    let mv = document.getElementById('main-view'); if (mv) mv.style.zoom = targetZoom;
    let slider = document.getElementById('zoom-slider'); if (slider) slider.value = targetZoom;
}
...
function changeLanguage(lang) {
    ...
    setTxt('tut-btn', t.tutBtn); const statusEl = document.getElementById('txt-status'); if (statusEl && statusEl.innerText.includes("未登入")) setTxt('txt-status', t.statusNotLogin); 
    setTxt('btn-unlock', t.btnUnlock); setTxt('tab-btn-link', t.tabLink); setTxt('tab-btn-text', t.tabText); setTxt('tab-btn-edit', t.tabEdit);
    let dc = document.getElementById('deck-code'); if(dc) dc.placeholder = t.phDeckCode; setTxt('btn-parse-official', t.btnParseOfficial);
    let dt = document.getElementById('deck-text'); if(dt) dt.placeholder = t.phDeckText; setTxt('btn-parse-text', t.btnParseText); 
    setTxt('lbl-search-db', t.lblSearchDb); let si = document.getElementById('search-input'); if(si) si.placeholder = t.phSearchInput;
    setTxt('lbl-custom-card', t.lblCustomCard); let ci = document.getElementById('custom-card-input'); if(ci) ci.placeholder = t.phCustomCard;
    setTxt('btn-add-custom', t.btnAddCustom); setTxt('txt-deck-list-title', t.deckListTitle); setTxt('btn-preview-deck', t.btnPreviewDeck); 
    setTxt('btn-start-game', t.btnStartGame); setTxt('btn-save-cloud', t.btnSaveCloud); setTxt('opt-load-cloud', t.optLoadCloud); 
    setTxt('txt-disclaimer', t.disclaimer, true); setTxt('lbl-zoom', t.lblZoom); setTxt('summary-prob-title', t.summaryProbTitle); 
    setTxt('txt-toggle-badge', t.txtToggleBadge); setTxt('lbl-step1', t.lblStep1); setTxt('lbl-rule-and', t.lblRuleAnd); 
    setTxt('lbl-rule-or', t.lblRuleOr); setTxt('btn-select-direct', t.btnSelectDirect); setTxt('lbl-draw1', t.lblDraw1); 
    setTxt('lbl-dead-hand', t.lblDeadHand); setTxt('lbl-step2', t.lblStep2); setTxt('btn-add-chain', t.btnAddChain); 
    setTxt('btn-run-sim', t.btnRunSim); setTxt('btn-save-scenario', t.btnSaveScenario); setTxt('btn-clear-board', t.btnClearBoard); 
    setTxt('title-prize', t.titlePrize); setTxt('title-stadium', t.titleStadium); setTxt('title-active', t.titleActive); 
    setTxt('title-bench', t.titleBench); setTxt('title-hand', t.titleHand); setTxt('txt-hand-sub', t.txtHandSub); 
    setTxt('btn-hand-shuffle', t.btnHandShuffle); setTxt('btn-hand-discard', t.btnHandDiscard); setTxt('title-deck', t.titleDeck); 
    setTxt('btn-draw-card', t.btnDrawCard); setTxt('title-discard', t.titleDiscard); setTxt('btn-export-game', t.btnExportGame); 
    setTxt('btn-import-game', t.btnImportGame); setTxt('btn-undo', t.btnUndo); setTxt('btn-redo', t.btnRedo); 
    setTxt('modal-auth-title', t.modalAuthTitle); setTxt('btnCloseAuth', t.btnCloseAuth); setTxt('modal-share-title', t.modalShareTitle); 
    setTxt('modal-share-desc', t.modalShareDesc); setTxt('btn-share-close', t.btnShareClose); setTxt('btn-share-copy', t.btnShareCopy); 
    setTxt('btn-gallery-close', t.btnGalleryClose); setTxt('gallery-confirm-btn', t.galleryConfirmBtn); setTxt('tut-main-title', t.tutMainTitle); 
    setTxt('btn-tut1', t.btnTut1); setTxt('btn-tut2', t.btnTut2); setTxt('btn-tut3', t.btnTut3); setTxt('tut1-desc', t.tut1Desc); 
    setTxt('btn-tut1-a', t.btnTut1A); setTxt('btn-tut1-b', t.btnTut1B); setTxt('btn-tut1-c', t.btnTut1C); setTxt('tut1-a-desc', t.tut1ADesc); 
    setTxt('tut1-b-desc', t.tut1BDesc); setTxt('tut1-c-desc', t.tut1CDesc); setTxt('tut2-h4', t.tut2H4); setTxt('tut2-desc', t.tut2Desc); 
    setTxt('tut3-h4', t.tut3H4); setTxt('tut3-desc', t.tut3Desc); setTxt('btn-tut-ready', t.btnTutReady); setTxt('bench-size-label', `${benchSize} ${t.benchLabel}`);
...
// 4. 動畫、教學導覽與戰術工具
// ==========================================
function openProbTutorial() { document.getElementById('prob-tutorial-modal').style.display = 'flex'; }
let splashDismissed = false;
function dismissSplash() { if (splashDismissed) return; splashDismissed = true; const splash = document.getElementById('octopus-splash'); if (splash) { splash.style.opacity = '0'; splash.style.visibility = 'hidden'; setTimeout(() => splash.remove(), 800); } }
function runSplashAnimation() {
    ...
    if (!countEl) return;
    let progress = 0; let currentProb = 0.0; let targetProb = 47.0; let totalSteps = 5000 / 30; let stepProb = targetProb / (totalSteps * 0.7);
    let timer = setInterval(() => {
        ...
        if (progress > 30 && progress <= 65) { if (phaseEl) phaseEl.innerText = "🎯 萬次蒙地卡羅 ✕ 深度勝率精算"; } else if (progress > 65) { if (phaseEl) phaseEl.innerText = "🏆 WIN MORE：掌舵對局，勝率大增"; }
...
function showTourStep(index) {
    ...
    if (targetEl) {
        ...
        if (step.id === 'history-controls') {
            box.style.display = 'block'; box.style.top = (window.innerHeight - (box.offsetHeight || 220) - 80) + 'px'; box.style.left = (window.innerWidth - (box.offsetWidth || 320) - 30) + 'px';
        } else {
            targetEl.scrollIntoView({ behavior: 'smooth', block: 'center' }); box.style.display = 'block';
...
function rollDice() { let res = Math.floor(Math.random() * 6) + 1; let display = res === 1 ? '🐙' : res; showBigResult(display, res === 1 ? '#00E5FF' : '#FFD700'); }
function flipCoin() { let res = Math.random() < 0.5 ? '正' : '反'; showBigResult(res, res === '正' ? '#FF5252' : '#888'); }
function showBigResult(text, color) { let modal = document.getElementById('dice-modal'); let resEl = document.getElementById('dice-result'); resEl.innerText = text; resEl.style.color = color; resEl.classList.remove('bounce-in'); void resEl.offsetWidth; resEl.classList.add('bounce-in'); modal.style.display = 'flex'; setTimeout(() => { modal.style.display = 'none'; }, 2000); }

const octoQuotes = [ "Replay. Analyze. Win More! 今天也要贏！", "萬次蒙地卡羅算力已就緒！", "對局有精采操作？點右下角一鍵分享給朋友！", "需要幫忙嗎？可以點擊右上角「❓ 教學」喔！", "我是小章魚，為你的勝率保駕護航！", "覆盤是通往大師連勝唯一的捷徑！" ];
let isMascotDragging = false; let mascotOffsetX = 0, mascotOffsetY = 0; let speechBubbleTimer = null;
function triggerOctoMascotClick(e) {
    if (isMascotDragging) return; const bodyEl = document.getElementById('octo-3d-body'); const bubble = document.getElementById('octo-speech-bubble');
    if (bodyEl) { bodyEl.classList.remove('jump'); void bodyEl.offsetWidth; bodyEl.classList.add('jump'); }
    if (bubble) { bubble.innerText = octoQuotes[Math.floor(Math.random() * octoQuotes.length)]; bubble.classList.add('active'); clearTimeout(speechBubbleTimer); speechBubbleTimer = setTimeout(() => { bubble.classList.remove('active'); }, 3500); }
}
function startMascotDrag(e) {
    const mascot = document.getElementById('octo-mascot-wrapper'); if (!mascot) return;
    ...
    function onMove(moveEvent) {
        let curX = moveEvent.clientX || (moveEvent.touches && moveEvent.touches[0].clientX); let curY = moveEvent.clientY || (moveEvent.touches && moveEvent.touches[0].clientY);
...
function checkAgreements() {
    ...
    if (btn && rem) {
        ...
        if (termsAgreed && privacyAgreed) { rem.style.color = '#34A853'; rem.innerText = '✅ 條款皆已同意，可以發送驗證連結了！'; if (!inCooldown) btn.disabled = false; } else { btn.disabled = true; rem.style.color = '#FF5252'; rem.innerText = '⚠️ 請先點擊閱讀並同意上述兩項條款，方可發送'; }
    }
}
function checkCooldownOnLoad() { let lastSend = localStorage.getItem('magic_link_last_send'); let count = parseInt(localStorage.getItem('octoplus_send_count')) || 0; if (lastSend) { let diff = Math.floor((new Date().getTime() - parseInt(lastSend)) / 1000); let targetCooldown = count === 1 ? 5 : (count === 2 ? 30 : 300); if (diff < targetCooldown) startCooldownTimer(targetCooldown - diff); } }
function startCooldownTimer(seconds) { const btn = document.getElementById('send-verify-btn'); if (!btn) return; btn.disabled = true; let countdown = seconds; btn.innerText = `⏳ 冷卻中 (${countdown}s)`; const timer = setInterval(() => { countdown--; if (countdown > 0) { btn.innerText = `⏳ 冷卻中 (${countdown}s)`; } else { clearInterval(timer); btn.innerText = "✨ 發送驗證連結"; checkAgreements(); } }, 1000); }

window.addEventListener('load', () => {
    checkAgreements(); checkCooldownOnLoad(); applyAutoZoom(); changeLanguage(currentLang);
    try { let savedDeck = localStorage.getItem('octoplus_deck'); if (savedDeck) { deckDict = JSON.parse(savedDeck); updateDeckUI(); } let savedBoard = localStorage.getItem('octoplus_board'); if (savedBoard) gameCards = JSON.parse(savedBoard); } catch(e) {}
...
if (supabaseClient) { supabaseClient.auth.onAuthStateChange((event, session) => { if (session) handleSession(session); }); }

async function handleSession(session) {
    document.getElementById('gate-overlay').style.display = 'none'; document.getElementById('user-id-input').value = session.user.email ? session.user.email.split('@')[0] : session.user.id.substring(0, 8); document.getElementById('btn-unlock').innerText = "訂閱、輸入邀請碼";
    try {
        if (supabaseClient) {
            ...
            if (data) {
                ...
                if (data.is_pro || hasActiveSub) {
                    let expStr = "終身尊榮 VIP ♾️"; if (data.pro_expires_at) { let d = new Date(data.pro_expires_at.replace("Z", "+00:00")); if (d.getFullYear() < 2090) expStr = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`; }
                    document.getElementById('txt-status').innerHTML = `👑 <b>Pro 專業會員</b><br><span style="font-size:11px; color:#FFD700;">(有效期限: ${expStr})</span>`; document.getElementById('auth-status-bar').style.background = "rgba(255, 215, 0, 0.12)"; document.getElementById('auth-status-bar').style.border = "1px solid rgba(255, 215, 0, 0.5)";
                } else {
                    document.getElementById('txt-status').innerHTML = `<b>免費試用版</b><br><span style="font-size:11px; color:#00E5FF;">(每日 30 次額度)</span>`; document.getElementById('auth-status-bar').style.background = "rgba(33, 38, 44, 1)"; document.getElementById('auth-status-bar').style.border = "1px solid #30363D";
                }
            }
        }
    } catch (err) {}
    fetchSavedDecks(); if (localStorage.getItem('octoplus_tour_done') !== 'Y') setTimeout(() => { startInteractiveTour(); }, 600);
}

async function loginWithMagicLink() {
    let termsAgreed = localStorage.getItem('octoplus_terms_agreed') === 'Y'; let privacyAgreed = localStorage.getItem('octoplus_privacy_agreed') === 'Y';
    if (!termsAgreed || !privacyAgreed) return alert("請先閱讀並同意服務條款與隱私權政策！"); if (!supabaseClient) return alert("系統初始化失敗，請重新整理網頁");
    const email = (document.getElementById('magic-email-input') ? document.getElementById('magic-email-input').value.trim() : ""); if (!email) return alert("請輸入有效的 Email 信箱");
    const btn = document.getElementById('send-verify-btn'); const msgEl = document.getElementById('magic-link-msg'); btn.innerText = "⏳ 發送中..."; btn.disabled = true; msgEl.style.display = 'none';
    try {
        ...
        if (error) { msgEl.style.color = "#FF5252"; msgEl.innerText = "❌ 發送失敗：" + error.message; msgEl.style.display = 'block'; btn.innerText = "✨ 發送驗證連結"; btn.disabled = false; } 
            ...
            let now = new Date().getTime(); let count = parseInt(localStorage.getItem('octoplus_send_count')) || 0; if (now - (parseInt(localStorage.getItem('magic_link_last_send')) || 0) > 60 * 60 * 1000) count = 0;
            localStorage.setItem('octoplus_send_count', ++count); localStorage.setItem('magic_link_last_send', now); startCooldownTimer(count === 1 ? 5 : (count === 2 ? 30 : 300));
        }
...
async function checkLoginStatus() { if (!supabaseClient) return false; const { data: { session } } = await supabaseClient.auth.getSession(); if (!session) { document.getElementById('gate-overlay').style.display = 'flex'; return false; } return session.access_token; }
...
function updateDeckUI() {
    ...
    document.getElementById('deck-total-count').innerText = total; document.getElementById('deck-total-count').style.color = (total === 60) ? '#00E5FF' : '#FF5252'; checkBlink();
...
async function parseText() {
    ...
    for(let line of lines) {
        ...
        if(match) {
            ...
            if(localCache[finalCardKey]) { if(!newDeck[finalCardKey]) newDeck[finalCardKey] = {qty: 0, img: localCache[finalCardKey].img, name: searchName, fallback_img: localCache[finalCardKey].fallback_img}; newDeck[finalCardKey].qty += parseInt(match[1]); } else { unknownText.push(line); }
...
function debounceSearch(val) {
    clearTimeout(searchTimeout); if(!val) { document.getElementById('search-results').style.display='none'; return; }
    searchTimeout = setTimeout(() => {
        fetch(`${API_BASE}/api/v1/search_db?q=${val}`).then(r=>r.json()).then(d=>{
            let resBox = document.getElementById('search-results'); resBox.innerHTML = "";
            if(d.results && d.results.length > 0) { d.results.forEach(c => { let div = document.createElement('div'); div.className = 'search-result-item'; div.innerHTML = `<img src="${c.img}" onerror="this.onerror=null; this.src='${DEFAULT_CARDBACK}'"> <span style="font-size:13px;">${c.name}</span>`; div.onclick = () => { if(!deckDict[c.key]) deckDict[c.key] = {qty:0, img: c.img, name: c.name, fallback_img: DEFAULT_CARDBACK}; deckDict[c.key].qty++; updateDeckUI(); document.getElementById('search-input').value = ""; resBox.style.display='none'; }; resBox.appendChild(div); }); resBox.style.display = 'block'; } else { resBox.style.display = 'none'; }
        });
    }, 300);
}
...
function changeCardBack(color) {
    ...
    renderBoard(); if(document.getElementById('gallery-modal').style.display === 'flex') { let items = currentModalMode === 'preview' ? Object.keys(deckDict).map(k => ({ key: k, ...deckDict[k] })) : []; if(items.length) renderGalleryItems(items); }
}
...
function handleCardClick(imgUrl, fallbackUrl) { if(!isDragging) { let safeImg = (imgUrl && imgUrl.startsWith('http')) ? imgUrl : DEFAULT_CARDBACK; let fUrl = (fallbackUrl && fallbackUrl.startsWith('http')) ? fallbackUrl : DEFAULT_CARDBACK; let lbImg = document.getElementById('lightbox-img'); lbImg.src = safeImg; lbImg.onerror = function() { this.onerror = null; this.src = fUrl; }; document.getElementById('lightbox-modal').style.display = 'flex'; } }
...
function openSelector(type) {
    ...
    else if (type.startsWith('chain_target_')) { let parentKey = type.replace('chain_target_', ''); tempSelectedKeys = chainList[parentKey].targets ? Object.keys(chainList[parentKey].targets) : []; } 
    else tempSelectedKeys = [];
...
function renderGalleryItems(items) {
    ...
    items.forEach(item => {
        const div = document.createElement('div'); let isSelected = currentModalMode.startsWith('search_multi') ? (searchMultiSelection[item.key] && searchMultiSelection[item.key] > 0) : tempSelectedKeys.includes(item.key); div.className = 'gallery-item' + (isSelected ? ' selected' : '');
        ...
        div.onclick = () => {
            ...
            if(currentModalMode === 'direct' || currentModalMode.startsWith('chain_target_') || currentModalMode === 'prize_target') { if(tempSelectedKeys.includes(item.key)) { tempSelectedKeys = tempSelectedKeys.filter(k=>k!==item.key); div.classList.remove('selected'); } else { tempSelectedKeys.push(item.key); div.classList.add('selected'); } } 
...
function modifySearchQty(key, delta, maxQty) {
    if(!searchMultiSelection[key]) searchMultiSelection[key] = 0; searchMultiSelection[key] += delta; if(searchMultiSelection[key] < 0) searchMultiSelection[key] = 0; if(searchMultiSelection[key] > maxQty) searchMultiSelection[key] = maxQty;
    let zone = currentModalMode.split('_')[2]; let arr = gameCards.filter(c => c.zone === zone); let groups = {};
    arr.forEach(c => { if(!groups[c.key]) groups[c.key] = {max_qty:0, img:c.img, name:c.name, key:c.key}; groups[c.key].max_qty++; });
...
function confirmGallerySelection() {
    ...
    else if (currentModalMode === 'prize_target') { let newList = {}; tempSelectedKeys.forEach(k => { newList[k] = prizeTargetList[k] || { name: deckDict[k].name, img: deckDict[k].img }; }); prizeTargetList = newList; renderPrizeTargetUI(); }
...
function renderPrizeTargetUI() {
    const container = document.getElementById('prize-target-display'); container.innerHTML = ""; let keys = Object.keys(prizeTargetList); if(keys.length === 0) { container.innerHTML = `<div style="color:#888; text-align:center;">未選擇任何卡片</div>`; return; }
    keys.forEach(k => {
        let c = prizeTargetList[k]; let div = document.createElement('div'); div.className = 'target-row'; let safeImg = (c.img && c.img.startsWith('http')) ? c.img : DEFAULT_CARDBACK;
        div.innerHTML = `<div style="display:flex; align-items:center;"><img src="${safeImg}" onerror="this.src='${DEFAULT_CARDBACK}'" style="width:30px;height:42px;margin-right:10px;"> <span style="font-weight:bold; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; max-width:200px;">${c.name}</span></div> <button class="btn-secondary" style="width:28px; height:28px; padding:0; border-radius:50%; color:#FF5252; display:flex; justify-content:center; align-items:center;" onclick="removePrizeTarget('${k.replace(/'/g, "\\\\'")}')">✕</button>`;
        container.appendChild(div);
    });
}
function removePrizeTarget(k) { delete prizeTargetList[k]; renderPrizeTargetUI(); }
...
function renderChainUI() {
    ...
    keys.forEach(k => {
        let c = chainList[k]; let div = document.createElement('div'); div.className = 'target-row'; div.style.flexDirection = 'column'; div.style.alignItems = 'stretch'; div.style.borderLeftColor = '#FFD700'; let isSearch = c.type.includes('檢索') || c.type.includes('Search'); let safeImg = (c.img && c.img.startsWith('http')) ? c.img : DEFAULT_CARDBACK; let safeKey = k.replace(/'/g, "\\\\'");
        div.innerHTML = `
...
function changeBenchSize(delta) {
    if (delta < 0) {
        let benchedCards = gameCards.filter(c => c.zone.startsWith('bench_')); let newSize = benchSize - 1; let occupiedSlots = new Set(benchedCards.map(c => c.zone));
        if (occupiedSlots.size > newSize) { showBenchShrinkModal(benchedCards, newSize); return; }
    }
    benchSize = Math.max(1, Math.min(8, benchSize + delta)); document.getElementById('bench-size-label').innerText = `${benchSize} 格`;
    renderBenchSlots(); renderBoard();
}

function showBenchShrinkModal(benchedCards, newSize) {
    let container = document.getElementById('bench-shrink-options'); container.innerHTML = ''; let zones = {};
    benchedCards.forEach(c => { if(!zones[c.zone]) zones[c.zone] = []; zones[c.zone].push(c); });
    Object.keys(zones).forEach(zName => {
        let stack = zones[zName]; let topCard = stack[stack.length - 1]; 
        let div = document.createElement('div'); div.style = "cursor: pointer; transition: 0.2s; border: 2px solid transparent; border-radius: 6px; padding: 2px;";
        div.onmouseover = () => div.style.borderColor = "#FF5252"; div.onmouseout = () => div.style.borderColor = "transparent";
        ...
        div.onclick = () => {
            stack.forEach(c => { c.zone = 'discard'; c.damage = 0; c.status = []; });
            document.getElementById('bench-shrink-modal').style.display = 'none';
            let remainingBenched = gameCards.filter(c => c.zone.startsWith('bench_')); let currentZones = [...new Set(remainingBenched.map(c => c.zone))].sort();
            currentZones.forEach((oldZone, idx) => { let newZone = `bench_${idx}`; if (oldZone !== newZone) { gameCards.filter(c => c.zone === oldZone).forEach(c => c.zone = newZone); } });
            benchSize = newSize; document.getElementById('bench-size-label').innerText = `${benchSize} 格`;
...
function renderBenchSlots() {
    ...
    for(let i=0; i<benchSize; i++) { let div = document.createElement('div'); div.id = `zone-bench-${i}`; div.className = "drop-zone drop-zone-stacked"; div.style.width = "150px"; div.ondrop = drop; div.ondragover = allowDrop; container.appendChild(div); }
}
...
function createCardEl(c, isField=false, isPrizeFaceUp=null) {
    ...
    if(isField) { inner += `<div class="bring-front-btn" title="移到最上層" onclick="event.stopPropagation(); bringToFront('${c.id}')">🔼置頂</div>`; }
...
function renderBoard() {
    ...
    fieldZones.forEach(zName => {
        ...
        arr.forEach((c, idx) => {
            ...
            if (c.damage > 0) { let dmgEl = document.createElement('div'); dmgEl.className = 'token-dmg'; dmgEl.innerText = c.damage; dmgEl.onclick = (e) => { e.stopPropagation(); c.damage = 0; saveState(); renderBoard(); }; el.appendChild(dmgEl); }
            if (c.status && c.status.length > 0) {
                let stContainer = document.createElement('div'); stContainer.className = 'token-status-container';
                c.status.forEach(s => { let stEl = document.createElement('div'); stEl.className = 'token-status'; const sMap = { 'status_poison': '☠️', 'status_burn': '🔥', 'status_confusion': '💫', 'status_paralysis': '⚡', 'status_sleep': '💤' }; stEl.innerText = sMap[s] || s; stEl.onclick = (e) => { e.stopPropagation(); c.status = c.status.filter(x => x !== s); saveState(); renderBoard(); }; stContainer.appendChild(stEl); });
        ...
        if (arr.length > 1) {
            let dragHandle = document.createElement('div'); dragHandle.className = 'stack-drag-handle'; dragHandle.draggable = true; dragHandle.innerHTML = '🔗 整疊拖曳';
            dragHandle.ondragstart = (e) => { isDragging = true; e.dataTransfer.setData("stack_zone", zName); setTimeout(() => { domEl.style.opacity = '0.3'; }, 0); };
            dragHandle.ondragend = () => { isDragging = false; domEl.style.opacity = '1'; renderBoard(); }; domEl.appendChild(dragHandle);
        }
    });
...
function drop(ev) {
    ev.preventDefault(); ev.currentTarget.classList.remove('dragover'); let targetZone = ev.currentTarget.id.replace('zone-', '').replace('-', '_');

    let stackZone = ev.dataTransfer.getData("stack_zone");
    if (stackZone) {
        if (stackZone !== targetZone) { gameCards.filter(c => c.zone === stackZone).forEach(c => { c.zone = targetZone; if (targetZone !== 'active' && !targetZone.startsWith('bench_')) { c.damage = 0; c.status = []; } gameCards.push(gameCards.splice(gameCards.indexOf(c), 1)[0]); }); saveState(); setTimeout(() => renderBoard(), 10); } return;
    }
    ...
    if (tokenData) {
        if (targetZone === 'active' || targetZone.startsWith('bench_')) {
            ...
            if (topCard) {
                ...
                else if (tokenData.startsWith('heal_')) { topCard.damage = Math.max(0, (topCard.damage || 0) - parseInt(tokenData.split('_')[1])); } 
                else if (tokenData.startsWith('status_')) { topCard.status = topCard.status || []; if (!topCard.status.includes(tokenData)) topCard.status.push(tokenData); }
    ...
    if (cardId) {
        ...
        if(c) {
            ...
            c.zone = targetZone; gameCards.push(gameCards.splice(gameCards.indexOf(c), 1)[0]); saveState(); setTimeout(() => renderBoard(), 10);
        }
    }
}
document.querySelectorAll('.drop-zone').forEach(z => z.addEventListener('dragleave', (e) => e.currentTarget.classList.remove('dragover')));

function shuffleDeck() { let deckCards = gameCards.filter(c => c.zone === 'deck'); if (deckCards.length <= 1) return alert("⚠️ 牌庫卡片不足（0 或 1 張），無需洗牌！"); deckCards.sort(() => Math.random() - 0.5); gameCards = gameCards.filter(c => c.zone !== 'deck').concat(deckCards); saveState(); renderBoard(); let msg = document.getElementById('marquee-text'); if (msg) { msg.innerHTML = "🔀 牌庫已重新完成隨機洗牌！"; msg.style.color = "#FFD700"; } }
...
function drawRandomPrize() {
    ...
    if (msg) { msg.innerHTML = `🎁 盲抽獎賞卡：【${drawnCard.name}】 已進入手牌！`; msg.style.color = "#00C9FF"; }
}
...
function calcPrizeProb() {
    ...
    if (K === 0) { prob = 0; } 
    else if (draws >= N - K + 1) { prob = 100; } 
    else { let pX0 = combination(N - K, draws) / combination(N, draws); prob = (1 - pX0) * 100; }
    
    let resEl = document.getElementById('prize-prob-result');
    resEl.innerText = `${prob.toFixed(1)} %`;
    resEl.style.color = prob > 0 ? '#00E5FF' : '#FF5252';
    resEl.classList.remove('bounce-in'); void resEl.offsetWidth; resEl.classList.add('bounce-in');
}
...
async function runSimulation() {
    let token = await checkLoginStatus(); if (!token) return;
    ...
    fetch(`${API_BASE}/api/v1/simulate`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` }, body: JSON.stringify({ deck_cards: gameCards.filter(c => c.zone === 'deck').map(c => ({name: c.key})), direct_targets: directDict, chain_targets: formattedChainDict, draw1: d1, target_rule: targetRule, dead_hand_size: deadHand }) }).then(async r => {
        ...
        if(!r.ok) { const errData = await r.json().catch(() => ({})); throw new Error(errData.detail || `伺服器錯誤 (${r.status})`); } return r.json();
    }).then(d => {
        if(d.success) {
...
function saveScenario() {
    if(!lastSimResult) return; const b = document.getElementById('ab-board'); let maxProb = Math.max(...Array.from(b.children).map(c => parseFloat(c.dataset.prob)||0), lastSimResult.prob);
    let div = document.createElement('div'); div.className = 'ab-item'; div.dataset.prob = lastSimResult.prob; div.style.borderTopColor = (lastSimResult.prob >= maxProb && lastSimResult.prob > 0) ? '#E53935' : '#444';
...
function exportGameState() {
    if(gameCards.length === 0) return alert("尚未開局！請先點擊左下角「鎖定牌組並開局」。"); let ruleEl = document.querySelector('input[name="target_rule"]:checked');
    fetch(`${API_BASE}/api/v1/share_game`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ game_data: { cards: gameCards.map(c => ({ k: c.key, z: c.zone, i: c.img, n: c.name, f: c.fallback_img })), deck: deckDict, targets: targetList, chains: chainList, draw1: parseInt(document.getElementById('draw1-qty').value) || 7, rule: (ruleEl ? ruleEl.value : 'AND'), dead: parseInt(document.getElementById('dead-hand-qty').value) || 0 } }) }).then(r => r.json()).then(d => {
        if(d.success) { document.getElementById('share-code-display').value = d.share_code; document.getElementById('share-modal').style.display = 'flex'; } else alert("分享失敗：" + (d.detail || "未知錯誤"));
...
async function saveDeckToDB() {
    ...
    try { const { data: { session } } = await supabaseClient.auth.getSession(); const { error } = await supabaseClient.from('user_decks').insert([{ user_id: session.user.id, deck_name: deckName, deck_data: deckDict }]); if (error) throw error; alert("💾 牌組儲存成功！"); fetchSavedDecks(); } catch (err) { alert("儲存失敗：" + err.message); }
}
...
async function fetchSavedDecks() {
    ...
    try { const { data, error } = await supabaseClient.from('user_decks').select('id, deck_name').order('created_at', { ascending: false }); if (error) throw error; let select = document.getElementById('saved-decks-select'); select.innerHTML = `<option value="">載入雲端牌組...</option>`; data.forEach(deck => { let opt = document.createElement('option'); opt.value = deck.id; opt.innerText = deck.deck_name; select.appendChild(opt); }); } catch (err) {}
}

async function loadDeckFromDB(deckId) {
    if (!deckId || !supabaseClient) return;
    try { const { data, error } = await supabaseClient.from('user_decks').select('deck_data').eq('id', deckId).single(); if (error) throw error; deckDict = data.deck_data; updateDeckUI(); alert("📥 牌組載入成功！"); document.getElementById('saved-decks-select').value = ""; } catch (err) { alert("載入失敗：" + err.message); }
}

async function fetchMarquee() {
    try { const resp = await fetch(`${API_BASE}/api/v1/marquee`); const data = await resp.json(); let text = data.text || "歡迎使用 PTCG 小章魚"; let el = document.getElementById('marquee-text'); el.innerHTML = text; el.style.animationDuration = Math.max(text.length * 0.45, 20) + 's'; } 
